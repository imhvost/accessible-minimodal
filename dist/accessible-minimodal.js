...
const $ = {
  ...
  const { modal: t, wrapp: i, body: e, active: s, open: o, close: a } = n.classNames, l = (m) => {
    switch (m) {
      case "top":
        return "0 auto auto";
      case "bottom":
...
class k {
  ...
  init() {
    var t, i;
    if ((t = this.config.style) != null && t.use && this.addStyles(), (i = this.config.hash) != null && i.open) {
      const e = window.location.hash.substring(1) ?? "";
      e && (this.modal = document.getElementById(e), this.openModal());
    }
  }
  addTriggers() {
    document.addEventListener("click", (t) => {
      ...
      ), this.focusBtns.push(this.openBtn), this.openModal()), i(((o = this.config.triggersAttrs) == null ? void 0 : o.close) ?? "") && (t.preventDefault(), this.closeModal()), i(((a = this.config.triggersAttrs) == null ? void 0 : a.closeAll) ?? "") && (t.preventDefault(), this.closeAllModals()), this.config.outsideClose) {
        const r = t.target;
        this.modal && r.classList.contains(((l = this.config.classes) == null ? void 0 : l.wrapp) ?? "") && !((d = document == null ? void 0 : document.activeElement) != null && d.closest("." + ((c = this.config.classes) == null ? void 0 : c.body))) && (t.preventDefault(), this.closeModal(this.modal));
      }
    });
  }
  openModal(t, i = !0) {
    ...
    if (this.animated)
      return;
    if (t && (typeof t == "string" ? this.modal = document.querySelector(t) : this.modal = t, i && this.focusBtns.push(null)), !this.modal) {
      ...
      (h = this.modal) == null || h.classList.add(((r = this.config.classes) == null ? void 0 : r.active) ?? ""), (m = this.modal) == null || m.setAttribute("aria-hidden", "false"), setTimeout(() => {
        ...
        if (document.addEventListener("keydown", this.onKeydown.bind(this)), (g = this.config.focus) != null && g.use && this.config.focus.selectors) {
          ...
          if (w) {
            let v = w[0];
            v.hasAttribute(
              ((b = this.config.triggersAttrs) == null ? void 0 : b.close) ?? ""
  ...
  closeModal(t, i = !0, e = !1) {
    var a, l, c, d, r;
    if (this.animated && !e)
      return;
    this.animated = !0;
    let s = null;
    if (t ? typeof t == "string" ? s = document.querySelector(t) : s = t : this.modal && (s = this.modal), !s) {
      this.animated = !1;
      return;
    }
  ...
  changeFocus(t) {
    var a, l;
    if (!((a = this.config.focus) != null && a.selectors))
      return;
    const i = (l = this.modal) == null ? void 0 : l.querySelectorAll(
      this.config.focus.selectors.join(", ")
    );
    if (!i)
      return;
    const e = i[0], s = i[i.length - 1], o = t.target;
    t.shiftKey ? o.isEqualNode(e) && (s.focus(), t.preventDefault()) : o.isEqualNode(s) && (e.focus(), t.preventDefault());
  }
  addStyles() {
    var o, a, l, c, d;
    const t = { ...this.config.classes }, i = this.config.animationDuration + "ms", e = `[${(o = this.config.triggersAttrs) == null ? void 0 : o.close}]`, s = A({
      classNames: t,
      animationDuration: i,
      margin: (a = this.config.style) == null ? void 0 : a.valign,
      transform: (l = this.config.style) == null ? void 0 : l.animation,
      closeSelector: e,
      width: (c = this.config.style) == null ? void 0 : c.width
    });
    document.head.insertAdjacentHTML("beforeend", `<style>${s}</style>`), document.querySelectorAll("." + ((d = this.config.classes) == null ? void 0 : d.modal)).forEach((r) => {
